#!/usr/bin/env bash
SCHEMA_BASE_URL=https://gitlab.com/SiLA2/sila_base/raw/modular_schema/schema/
SCHEMA_BASE_URL_MASTER=https://gitlab.com/SiLA2/sila_base/raw/master/schema/
...
grep -rnl "$SCHEMA_BASE_URL_MASTER" . --include *.xml | xargs sed -i 's|'"$SCHEMA_BASE_URL_MASTER"'|'"$LOCAL_SCHEMA_URI"'|g'
